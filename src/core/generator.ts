/**
 * Versioned Queens generation. `legacy` dispatches to the frozen original.
 * `profile-v2` samples a joint historical feature profile, reserves tiny and
 * interior regions, builds templates, repairs uniqueness, and selects a
 * candidate near that profile. V3 uses expanded history with rare singleton boards.
 * See docs/generator-v2.md and docs/generator-v3.md.
 * Browser-safe and stateless for profile versions: replay depends on all options,
 * generator version, and the profile corpus, never on earlier calls.
 */
import { calibratedProfiles, isCalibrationLayout, SINGLETON_BOARD_RATE } from './generator-calibration'
import { generatePuzzle as generateLegacyPuzzle, resetGeneratorHistory as resetLegacyHistory } from './generator-legacy'
import { historicalProfiles, isHistoricalLayout, profileBoard, profileDistance, type BoardProfile } from './generator-profile'
...
export type GeneratorVersion = 'legacy' | 'profile-v2' | 'profile-v3'

export interface GeneratorOptions {
  /** Frozen original algorithm or profile-guided generation. API default: profile-v2. */
  version?: GeneratorVersion
  /** Candidate target for profile versions. Continue to the attempt cap if difficulty is missed. Default 12. */
  candidates?: number
  /** Optional training-only profiles for held-out benchmarking. */
  ...
  seed?: string
  /**
   * Construction cap (v2 default 120, v3 default 180). Returns the closest unique
   * candidate with its true rating; throws if none exists within the cap.
   * Legacy retains its original default (60) and attempt semantics.
   */
  maxAttempts?: number
  /** Wanted difficulty; 'any' samples the historical difficulty with its profile. */
...
function tryCarve(b: Build, g: number, cells: number[]): boolean {
  if (b.interior?.[g] && cells.some(k => Math.floor(k / b.n) === 0 || Math.floor(k / b.n) === b.n - 1 || k % b.n === 0 || k % b.n === b.n - 1)) return false
...
function snakeCells(b: Build, g: number, rng: Rng, length: number): number[] | null {
  ...
  while (path.length < length && stuck < 2) {
    ...
    const end = b.profile && rng.next() < 0.4 ? rng.pick(path) : fromHead ? head : tail
    let options = freeNeighbours(b, end).filter((k) => !b.isQueen[k] && !inPath.has(k) && orthoInPath(k) === 1 && (!b.interior?.[g] || (Math.floor(k / n) > 0 && Math.floor(k / n) < n - 1 && k % n > 0 && k % n < n - 1)))
...
function carve(b: Build, g: number, rng: Rng, tries: number): boolean {
  for (let t = 0; t < tries; t++) {
    ...
    if (b.profile && b.role[g] === 'tiny' && cells?.length !== b.target[g]) continue
...
function fill(b: Build, rng: Rng): void {
  ...
  const frontierOf = (g: number): number[] => {
    ...
    for (let k = 0; k < n * n; k++) if (b.cell[k] === g) for (const j of freeNeighbours(b, k)) {
      if (!b.interior?.[g] || (Math.floor(j / n) > 0 && Math.floor(j / n) < n - 1 && j % n > 0 && j % n < n - 1)) out.add(j)
    }
    return [...out]
  }
  ...
  while (active.length) {
    ...
    if (!fr.length || ((g !== background || b.profile) && b.size[g] >= b.target[g])) {
...
export function buildRegions(
  ...
  const sizeRanges = sizeProfile(n)
  const order = rng.shuffle(Array.from({ length: n }, (_, i) => i))
  if (profile) {
    const available = new Set(order)
    const assignedOrder = new Array<number>(n).fill(-1)
    // Interior regions must start at interior queens. Assign those first.
    profile.regions.forEach((region, rank) => {
      if (!region.interior) return
      const choices = [...available].filter(g => g > 0 && g < n - 1 && sol[g] > 0 && sol[g] < n - 1)
      if (!choices.length) return
      const g = rng.pick(choices)
      assignedOrder[rank] = g
      available.delete(g)
    })
    for (let rank = 0; rank < n; rank++) {
      if (assignedOrder[rank] >= 0) continue
      const g = rng.pick([...available])
      assignedOrder[rank] = g
      available.delete(g)
    }
    order.splice(0, n, ...assignedOrder)
    b.interior = new Array<boolean>(n).fill(false)
    order.forEach((g, rank) => { b.interior![g] = profile.regions[rank].interior })
  }
  const background = order[0]
  b.role[background] = 'background'
  const tinyCount = Math.max(0, Math.min(profile ? profile.regions.filter(r => r.area <= 3).length : style.tinyCount, profile ? n - 1 : n - 2))
  ...
  if (profile) {
    order.forEach((g, rank) => {
      const region = profile.regions[rank]
      b.target[g] = region.area
      b.role[g] = rank === 0 ? 'background' : region.area <= 3 ? 'tiny'
        : region.shape === 'full-line' ? 'segment'
        : region.shape === 'single' || region.shape === 'domino' ? 'tiny' : region.shape
    })
  }
  // Reserve tiny regions before larger templates can squeeze them to singletons.
  const phases: Role[][] = profile
    ? [['tiny'], ['ring', 'rectangle', 'segment', 'L'], ['snake']]
    : [['ring', 'rectangle', 'segment', 'L'], ['snake'], ['tiny']]
  for (const phase of phases) {
    for (const g of shaped.concat(tinyRegions)) {
      ...
      if (!ok) {
        if (profile && b.role[g] === 'tiny') return Array.from({ length: n }, () => new Array<number>(n).fill(-1))
        if (b.role[g] === 'tiny') {
          ...
          b.role[g] = 'blob'
          if (!profile) b.target[g] = Math.max(3, Math.min(b.target[g], 8))
        }
      }
    }
  }
  ...
  if (targetsOut) targetsOut.push(...b.target)
...
export function repairUniqueness(
  ...
  options: {
    ...
    /** Profile-v2: preserve tiny regions and favor target areas during repair. */
  ...
  const legalMoves = (k: number, relaxed: boolean) =>
    movesForCell(regions, intended, sizes, k, 4, relaxed ? undefined : protectedRegions)
      .filter(m => !options.targetAreas || (sizes[m.to] > 3 && (relaxed || !protectedRegions.has(m.to))))
  ...
  const penalty = (m: Move) =>
    Math.min(
      ...
      (sizes[m.to] <= 3 ? 0.3 : 0) +
        ...
        (options.targetAreas ? 0.03 * (
          Math.abs(sizes[m.from] - m.cells.length - options.targetAreas[m.from]) +
          Math.abs(sizes[m.to] + m.cells.length - options.targetAreas[m.to])
        ) : 0),
    )
...
/** Reset legacy session history. Profile-v2 deliberately has no session state. */
...
/**
 * Generate with the selected version. Profile-v2 bounds construction work and
 * reports a difficulty mismatch instead of mislabelling its best candidate.
 */
export function generatePuzzle(options: GeneratorOptions = {}): Puzzle {
  if (options.version === 'legacy') {
    const puzzle = generateLegacyPuzzle(options)
    // Keep old unqualified IDs, but isolate progress for difficulty-specific
    // links: the frozen algorithm's ID did not include difficulty.
    return options.difficulty && options.difficulty !== 'any'
      ? { ...puzzle, id: `gen-legacy-${puzzle.size}-${options.difficulty}-${puzzle.attribution?.slice(5)}` }
      : puzzle
  }
  ...
  // Historical profiles cover 7–11 only. Preserve support for other sizes.
  if (size < 7 || size > 11) return generateLegacyPuzzle({ ...options, size, seed })
  // Preserve the implicit v2 API and every existing versioned seed.
  const version = options.version ?? 'profile-v2'
  const calibrated = version === 'profile-v3'
  const rng = makeRng(`${version}:${size}:${seed}:${wanted}`)
  let pool = (options.profiles ?? (calibrated ? calibratedProfiles() : historicalProfiles())).filter(p => p.size === size)
  if (calibrated) {
    const singleton = rng.next() < SINGLETON_BOARD_RATE
    pool = pool.filter(p => Boolean(p.features.hasSingleton) === singleton)
  }
  if (!pool.length) throw new Error(`No training profiles for size ${size}`)
  const matching = pool.filter(p => wanted === 'any' || p.difficulty === wanted)
  const target = rng.pick(matching.length ? matching : pool)
  const targetDifficulty = wanted === 'any' ? target.difficulty : wanted
  const candidates = Math.max(1, Math.min(100, Math.round(options.candidates ?? 12)))
  const maxAttempts = Math.max(1, Math.min(2000, Math.round(options.maxAttempts ?? (calibrated ? 180 : 120))))
  if (!Number.isFinite(candidates) || !Number.isFinite(maxAttempts)) throw new Error('Generation budgets must be finite')
  const id = `gen-${version}-${size}-${wanted}-${seed}`
  const seen = new Set<string>()
  let best: { regions: number[][]; difficulty: Difficulty; score: number; relaxed: boolean } | undefined
  let uniqueCandidates = 0
  let attempts = 0
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const solution = randomSolution(size, rng)
    const style = sampleStyle(rng, size, targetDifficulty)
    attempts++
    const roles: Role[] = []
    const targets: number[] = []
    // Do not let a geometrically unproductive sampled profile cause retries
    // forever. Keep its scoring target but broaden construction within the cap.
    const relaxed = !best && attempt >= Math.floor(maxAttempts / 2)
    const regions = buildRegions(size, solution, rng, style, undefined, roles, relaxed ? undefined : target, targets)
    if (regions.some(row => row.some(g => g < 0))) continue
    const protectedRegions = new Set<number>()
    roles.forEach((role, g) => { if (RIGID_ROLES.has(role)) protectedRegions.add(g) })
    if (!repairUniqueness(regions, solution, rng, { protectedRegions, targetAreas: relaxed ? undefined : targets })) continue
    const canonical = symmetricCanonicalKey(regions)
    if (seen.has(canonical) || (calibrated ? isCalibrationLayout(regions) : isHistoricalLayout(regions))) continue
    seen.add(canonical)
    const actual = profileBoard({ size, regions })
    if (calibrated && actual.features.hasSingleton !== target.features.hasSingleton) continue
    // A matched difficulty always beats a mismatch. Among matches retain the
    // best resemblance to the sampled joint profile, not the archive mean.
    const score = profileDistance(actual, target) + (actual.difficulty === targetDifficulty ? 0 : 100)
    if (!best || score < best.score) best = { regions, difficulty: actual.difficulty, score, relaxed }
    uniqueCandidates++
    if (uniqueCandidates >= candidates && best.score < 100) break
  }
  if (!best) throw new Error(`No unique ${version} ${size}×${size} puzzle within ${maxAttempts} attempts (seed ${seed})`)
  return {
    id, size, regions: normalizeRegions(best.regions), source: 'generated',
    name: puzzleName(size, best.difficulty), difficulty: best.difficulty,
    generation: { version, ...(calibrated ? { singletonAllowed: Boolean(target.features.hasSingleton) } : {}), relaxed: best.relaxed, targetDifficulty, difficultyMatched: best.difficulty === targetDifficulty, attempts, candidates: uniqueCandidates, distance: best.score - (best.difficulty === targetDifficulty ? 0 : 100) },
    attribution: `generator:${version}; seed:${seed}; requested:${wanted}; target:${targetDifficulty}; candidates:${uniqueCandidates}`,
  }
}
